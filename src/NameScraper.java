...
public class NameScraper {
	...
	public static void main(String[] args) throws MalformedURLException, IOException{
		...
		{
			List<Element> list = parseNameList(idList[x]);
	
			for(Element e : list)
			{
				System.out.print(x + "   " + e.getContent().toString());
			}
			System.out.println();
		}
  }
	...
	public static List<Element> parseNameList(int substanceId) throws MalformedURLException, IOException
	{
		String sourceUrlString="http://webwiser.nlm.nih.gov/getSubstanceData.do?substanceId=" + substanceId;
		String result = "";
		ArrayList<String> list = null;
		
		Source source=new Source(new URL(sourceUrlString));
		
        List<Element> elementList = source.getAllElements(HTMLElementName.H1);
        return elementList;
	}
}
